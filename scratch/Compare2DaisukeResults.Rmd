...
tmp <- which(sum_DODGLA$n.diff< -20)
sum_DODGLA[tmp,]

## for printing out the problems
crap <- sum_DODGLA[tmp,]
rownames(crap) <- crap$wbic_year
crap <- round(crap[,c("n.DO","n.DG","n.ratio","mnlen.diff","mnage.diff")],2)
crap[order(crap$n.ratio),]
```
...
plot(n.ratio~n.DO,data=sum_DODGLENS,pch=19,col=col2rgbt("black",1/5),
     ylab="Ratio (DG/DO) of n",xlab="Our n")
abline(h=1,lty=2,col="red")
